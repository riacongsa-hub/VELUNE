...
$(function () {
    ...
    $(".collage_menu li a").click(function (e) {
        ...
        // 탭을 바꿀 때 확대 상태 초기화
        $(".collage_wrap ul li img").removeClass("zoom");
    });


    // 스크롤로 이미지가 화면에 들어왔을 때 확대
    const observer = new IntersectionObserver((entries) => {

        entries.forEach((entry) => {

            if (entry.isIntersecting) {
                $(entry.target).addClass("zoom");
            }
    ...
    // 이미지
    $(".collage_wrap ul li img").each(function () {
        observer.observe(this);
    });

    // span
    $(".collage_wrap span").each(function () {
        observer.observe(this);
    });
